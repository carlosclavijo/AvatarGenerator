 ...
 * @author Carlos
 */
public class HatDaoJson extends HatDao{

    @Override
    public void insert(Hat obj) {
        try {
            ConnectionJson con = new ConnectionJson("Hat");
            FileReader reader = con.getReader();
            File file= con.getFile();
            List<Hat> list = new Gson().fromJson(reader, new TypeToken<List<Hat>>() {}.getType());
            list.add(obj);
            ...
            Logger.getLogger(HatDaoJson.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    @Override
    public void update(Hat obj) {
        try {
            ConnectionJson con = new ConnectionJson("Hat");
            FileReader reader = con.getReader();
            File file= con.getFile();
